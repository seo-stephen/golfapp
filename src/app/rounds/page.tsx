  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-cream-400 text-sm mt-1">Your round history.</p>
      </div>
        ...
        <ul className="divide-y divide-pine-800">
          {(rounds ?? []).map((r) => {
            ...
            return (
              <li key={r.id}>
                <Link
                  href={`/round?id=${r.id}`}
                  className="flex items-center justify-between py-3 min-h-14 active:opacity-60 sm:hover:text-kelly-400"
                >
                  <div>
          ...
          {(rounds ?? []).length === 0 && (
            <p className="text-sm text-cream-500 py-3">
              No rounds yet —{" "}
              <Link href="/round/new" className="text-kelly-400 underline">
                start one
              </Link>
              .
            </p>
